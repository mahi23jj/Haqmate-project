...
export const createMultiorderSchema = z.object({
    ...
    shippinglocation: z.string().min(5, {message: "Location is too short"}).max(200, {message: "Location is too long"}).optional(),
    phoneNumber: z.string().min(7, {message: "Phone number is too short"}).max(15, {message: "Phone number is too long"}).optional(),
    locationChange: z.boolean().optional(),
    phoneChange: z.boolean().optional()
})
