...
async function requestAuthToken(fabricToken, appToken) {
    ...
    const response = await axios.post(`${config.baseUrl}/payment/v1/auth/authToken`, reqObject, {
        headers: {
...
function createAuthRequestObject(appToken) {
    const req = {
...
export const createOrder = async (req, res) => {
    try {
        ...
        const { token: fabricToken } = await applyFabricToken();
        const createOrderResult = await requestCreateOrder(fabricToken, title, amount, order.merchOrderId);
...
// ---------------------------
export const requestCreateOrder = async (fabricToken, title, amount, merchOrderId) => {
    try {
        const reqObject = createRequestObject(title, amount, merchOrderId);
        console.log("Request Object:", reqObject);
        const response = await axios.post(`${config.baseUrl}/payment/v1/merchant/preOrder`, reqObject, {
            headers: {
                "Content-Type": "application/json",
                "X-APP-Key": config.fabricAppId,
                Authorization: fabricToken,
...
// ---------------------------
function createRequestObject(title, amount, merchOrderId) {
...
//   };
// }
//# sourceMappingURL=telebirr_paymentroute.js.map