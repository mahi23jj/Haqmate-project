...
export class OrderServiceImpl {
    async getOrdersWithTracking(userId, status, page = 1, limit = 20) {
    ...
    // 1. Get Orders with Tracking
    // ---------------------------
    async updateTrackingStep(orderId, type, message) {
    ...
    async updateOrderStatus(orderId, paymentStatus, deliveryStatus, deliveryDate) {
    ...
    async getAllOrders(status, paymentStatus, deliveryStatus) {
        const whereClause = {};
        if (status)
    ...
    // ---------------------------
    async createMultiOrder({ userId, products, locationId, // maps to areaId
    ...
    paymentMethod, idempotencyKey, extraDistance, }) {
        ...
        if (existingOrder)
            return { id: existingOrder.id, totalAmount: existingOrder.totalAmount };
        const deliveryService = new DeliveryServiceImpl();
        const productIds = products.map(p => p.productId);
        console.log("Fetching prices for products:", products);
        // Fetch prices (directly from DB)
        async function getProductPrices(ids) {
            const dbProducts = await prisma.teffProduct.findMany({
                where: { id: { in: ids } },
                select: { id: true, pricePerKg: true }
            });
