...
export class CartServiceImpl {
    calculateItemPrice(pricePerKg, quantity, packagingSize, discount) {
        const raw = pricePerKg *
            quantity *
            packagingSize *
            (1 - (discount ?? 0) / 100);
    ...
    async preloadCartOnLogin(userId) {
    ...
    async fetchProductsFromCacheOrDb(productIds) {
    ...
    async updateCartItemQuantity(userId, productId, packagingSize, quantity) {
    ...
    async updateCartItem(userId, cartId, newProductId, newPackaging, newQuantity) {
