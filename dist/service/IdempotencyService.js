...
export class IdempotencyService {
     ...
     */
    createRequestHash(operation, data) {
     ...
     */
    async checkIdempotency(key, operation, requestData) {
        ...
        if (record.requestHash !== requestHash) {
            throw new Error(`Idempotency key '${key}' already exists for a different request`);
        }
        return {
            id: record.id,
            key: record.key,
     ...
     */
    async storeIdempotency(key, operation, requestData, responseData, status, ttlHours) {
        const requestHash = this.createRequestHash(operation, requestData);
     ...
     */
    async updateIdempotencyStatus(key, operation, status, responseData) {
     ...
     */
    async getIdempotencyRecord(key, operation) {
        const record = await prisma.idempotencyKey.findFirst({
            where: { key, operation },
            orderBy: { createdAt: "desc" },
        });
...
export class CleanupService {
     ...
     */
    async cleanupExpiredPaymentIntents() {
